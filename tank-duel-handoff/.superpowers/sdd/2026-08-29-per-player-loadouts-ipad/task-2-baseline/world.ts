...
export interface CreateWorldOptions {
  ...
  /** Optional shell IDs after locked HE; shared by both hotseat players. */
...
export function createWorld(seed: number, options: CreateWorldOptions = {}): GameState {
  ...
  const defaultIds = STANDARD_WEAPONS
    .map((weapon) => weapon.shell.id)
    .filter((id) => id !== CONSTANTS.loadout.freeShell);
  const deck = equippedWeapons(createLoadout(options.loadoutIds ?? defaultIds));
  const makeArsenal = (): Arsenal => ({
    slots: deck,
  ...
  return {
    ...
    quietFrames: 0,
    roundResult: null,
    arsenals: [makeArsenal(), makeArsenal()],
    presentationEvents: [],
