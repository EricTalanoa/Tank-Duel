...
/**
 * The exact deck the loadout screen deploys for these picks: built through the same
 * `createLoadout` → `deploymentShellIds` path production uses, so it is legal by
 ...
 * the locked HE slot and the canonical `PLAYABLE_WEAPONS` order. Deliberately not the
 * default deck, so a runtime that ignored the supplied ids would be caught.
 */
