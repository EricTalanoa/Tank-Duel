...
export function createHowtoScene(
  ...
  const frame = (_timestamp: number): void => {
    frameHandle = null;
    if (disposed) return;
    const state = updateHowtoSceneModel(model, options.now() - startedAt);
...
export function drawHowtoScene(
  ...
  const colors = [PALETTE.playerOne, PALETTE.playerTwo, HE_SHELL.accent] as const;
  ...
  model.shots.forEach((shot, shotIndex) => {
    ...
    context.strokeStyle = colors[shotIndex % colors.length] ?? HE_SHELL.accent;
