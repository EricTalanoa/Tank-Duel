...
export function createPlayerLoadoutEditorModel(
  ...
  const loadouts: [Loadout, Loadout] = [
    ...
    ownedLoadout(initialPlayerLoadoutIds[1], enabledShellIds),
  ];
...
export function mountLoadout(root: HTMLElement, options: MountLoadoutOptions): MountedLoadout {
  ...
  const render = (): void => {
    const panels = element(document, 'div', 'loadout-panels');
    model.players.forEach((player, index) => panels.append(renderPanel(document, player, index)));
