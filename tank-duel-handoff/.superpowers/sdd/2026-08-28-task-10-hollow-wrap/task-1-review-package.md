...
No Git repository is present. Both production and test files below are new in Task 1.

## Changed files

- `src/sim/wrap.ts`
- `src/sim/wrap.test.ts`
...
import { nearestWrappedX, visibleCopyRange, wrapX, wrappedDelta } from './wrap';
...
describe('shared wrap coordinate helpers', () => {
  ...
  it('rejects non-positive and non-finite widths for every helper', () => {
    ...
    for (const helper of helpers) {
      ...
      expect(() => helper(Number.POSITIVE_INFINITY)).toThrow(/width must be a finite number greater than 0/);
    }
  });
});
```
