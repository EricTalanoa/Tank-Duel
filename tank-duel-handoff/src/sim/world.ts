...
import { makePlayerLoadouts, type PlayerIndex, type PlayerLoadouts } from './playerLoadouts';
...
export interface ResolvedShotImpact {
  readonly owner: PlayerIndex;
  readonly x: number;
  readonly y: number;
}
...
export interface GameState {
  ...
  lastResolvedShotImpact: ResolvedShotImpact | null;
  settleFrames: number;
  quietFrames: number;
...
/** Build one player's arsenal from their complete deck. Each call owns its slots and ammo. */
function makeArsenal(deckIds: readonly string[]): Arsenal {
...
function resolve(state: GameState): void {
  ...
  let resolvedShotImpact: ResolvedShotImpact | null = null;
  for (const pending of impacts) {
    ...
    if (impacts.length === 1) {
      resolvedShotImpact = Object.freeze({ owner: impact.owner, x: impact.x, y: impact.y });
    }
  }
  if (resolvedShotImpact) state.lastResolvedShotImpact = resolvedShotImpact;
