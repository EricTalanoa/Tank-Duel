...
describe('presentation registry', () => {
  ...
  it.each([
    ...
    {
      ...PRESENTATION,
      players: [{ ...PRESENTATION.players[0], unexpected: true }, PRESENTATION.players[1]],
    },
  ])('rejects an unexpected registry key: %j', (invalid) => {
    expect(() => validatePresentation(invalid)).toThrow();
  });
});
