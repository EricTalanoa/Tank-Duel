...
function renderMode(document: Document, state: AppFlowState, bind: Binder): HTMLElement {
  ...
  surface.append(list, ...cpuTierControls(document, state.config.mode, model.cpuTiers, bind));
  return surface;
}
...
function renderMap(document: Document, state: AppFlowState, bind: Binder): HTMLElement {
  ...
  for (const option of model.modeOptions) {
    modes.append(modeOptionButton(document, option, bind));
  }
  surface.append(modes, ...cpuTierControls(document, state.config.mode, model.cpuTiers, bind), grid);
  return surface;
}
...
function renderCustom(document: Document, state: AppFlowState, bind: Binder): HTMLElement {
  ...
  surface.append(
    ...
    form,
    actionButton(document, { label: 'Start match', action: model.startAction, disabled: false }, bind),
  );
  return surface;
}
...
function modeOptionButton(document: Document, option: ModeOptionModel, bind: Binder): HTMLButtonElement {
  const button = element(document, 'button', `menu-button mode-summary-option${option.selected ? ' is-selected' : ''}`) as HTMLButtonElement;
  button.setAttribute('type', 'button');
...
function cpuTierControls(
  ...
  const options = element(document, 'div', 'menu-grid');
  for (const tier of tiers) {
    const button = element(document, 'button', `menu-button map-tile cpu-tier-option${tier.selected ? ' is-selected' : ''}`) as HTMLButtonElement;
    button.setAttribute('type', 'button');
    ...
    button.append(textElement(document, 'span', tier.label));
